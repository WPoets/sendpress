...
if(isset($post) && false == get_post_meta( $post->ID , 'body_bg', true) ) {
	...
	update_post_meta( $post->ID , 'content_link',  get_post_meta( $default_post->ID , 'content_link', true) );
...
$content_link = array(
	'value' => get_post_meta( $post->ID , 'content_link', true),
	'std' => get_post_meta( $default_post->ID , 'content_link', true),
);
...
<div class="boxer-inner">
	<div class="leftcol">
		Body Background<br>
		<?php $this->create_color_picker( array('id'=>'body_bg','value'=>$body_bg['value'],'std'=>$body_bg['std'], 'link'=>'#html-view' ,'css'=>'background-color' ) ); ?>
		<br><br>
		Body Text Color<br>
		...
		<?php $this->create_color_picker( array('id'=>'header_bg','value'=>$header_bg['value'],'std'=>$header_bg['std'], 'link'=>'#html-header' ,'css'=>'background-color' ) ); ?>
		<br><br>
...
<br><br><hr><br>
		...
		<br><br>
		Content Link Color<br>
		<?php $this->create_color_picker( array('id'=>'content_link','value'=>$content_link['value'],'std'=>$content_link['std'],'link'=>'#html-content a' ,'css'=>'color' ) ); ?>
		<br><br>
	...
	<div class="widerightcol">
		<div id="imageaddbox" class="inputbox">
			...
			<label for="upload_image">Enter an URL or upload an image for the banner.<br>
				<input id="upload_image" type="text" size="36" name="upload_image" value="<?php echo $myimage; ?>"  /><a href="#" id="addimageupload" rel="<?php echo $post->ID; ?>" class="spbutton">Select Image</a><span class="error">Image path required to activate image</span>
			</label>
			<br><br>
			...
			<a href="" id="activate-image" class="spbutton"><?php if( $my_active_header === 'image' ){ echo 'Update'; }else{ echo 'Activate'; } ?> Image</a>
			<a href="" id="close-image" class="spbutton">Close</a>
		</div>
		<div id="textaddbox" class="inputbox">
			...
			<strong>URL:</strong><br><input type="text" name="header_link" value="<?php echo $my_header_link; ?>" style="width: 100%;"><br><br>
			...
			<a href="" id="activate-text" class="spbutton">Save and Activate</a> <a href="" id="save-text" class="spbutton">Save</a>
		</div>
					...
					<div id='html-header' class='header-holder empty'>
						...
						<div id="header-text"<?php if($my_active_header !== 'text'){ echo ' class="hide"'; } ?>>
							<div id="header-text-title">
								...
								<?php echo $my_header_text; ?>
								<?php if( strlen($my_header_link) > 0 ){ echo '</a>'; } ?>
							</div>
							<div id="header-text-tagline"><?php echo $my_sub_header_text; ?></div>
						</div>
						<div id="header-controls">
							<a href="" id="addimage" class="spbutton add">Image</a> <a href="" id="addtext" class="spbutton add">Text</a>
						</div>
					</div>
